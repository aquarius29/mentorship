...
function firmasite_enqueue_script() {
	...
	if (isset($firmasite_settings["font"]) && !empty($firmasite_settings["font"])){
		...
		wp_enqueue_style( 'google-webfonts', $firmasite_settings["protocol"] . '://fonts.googleapis.com/css?family=' . $firmasite_settings["font"] . ':300,400,700&amp;subset=latin'. $firmasite_data_subsets );
	...
	// if (visible_lg())
	add_action ("wp_footer", "firmasite_bootstrap_check_blocks",1);
	function firmasite_bootstrap_check_blocks() {
		?>
	...
	//throttle
	add_action ("wp_head", "firmasite_js_throttle",900);
	function firmasite_js_throttle() {
		?>
		<script type="text/javascript">
...
function firmasite_masonry_implement() {
	global $firmasite_settings, $firmasite_loop_tile_count;
		...
		(function ($) {
			...
			$(window).resize(throttle(function(){
				<?php if($firmasite_settings["loop_tile_row"] > 3){ ?>
					if((visible_md() || visible_lg()) && <?php echo $loop_tile; ?>.width() < 768){
					...
					if((visible_md() || visible_lg()) && <?php echo $loop_tile; ?>.width() < 480){
					...
					} else if((visible_md() || visible_lg()) && <?php echo $loop_tile; ?>.width() >= 480){
 ...
 */
if (true != FIRMASITE_DONT_SUGGEST_PLUGIN) {
