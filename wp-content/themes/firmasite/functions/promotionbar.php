	...
	if ( $firmasite_promotionbar->have_posts() ) {
		...
		switch($firmasite_settings["promotionbar-location"]) {
			case 'open_footer':
				...
				break;
			case 'open_sidebar':
				if (true == FIRMASITE_PROMOTIONBAR_BOTTOM) {
		...
		$wp_customize->add_section( 'promotionbar-settings', array(
			...
			$wp_customize->add_control( 'firmasite_settings[promotionbar-title]', array(
				...
				//'priority' => '99',
					...
					<script>
					jQuery(document).ready(function() {
						jQuery('li#customize-control-firmasite_settings-promotionbar-title label').prepend('<a href="#" class="popover-gogo pull-right" data-rel="popover" data-trigger="hover" data-placement="left" data-html="true" data-content="<?php 
